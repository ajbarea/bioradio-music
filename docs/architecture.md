...
End-to-end latency from a muscle activation to audible MIDI:

| Stage | Approx. time |
|---|---|
| EMG sample to BioRadio buffer | 4-10 ms (sensor + Bluetooth serial) |
| BioRadio to LSL stream | < 5 ms |
| 250 ms window collection (50% overlap means the classifier fires every 125 ms) | 125 ms |
| Feature extraction + classification | < 5 ms (RandomForest, 5 features) |
| Debounce gate (3 frames default) | up to 375 ms additional |
| `MidiController.on_classification()` -> note-on | < 1 ms |
| FluidSynth render -> WASAPI | typically < 10 ms |

The dominant term is the window + debounce path. Shrinking the window
or debounce count trades responsiveness for classifier stability; the
defaults are the team's tuned compromise. If the void feels sluggish,
drop `debounce_frames` to 2 first, then 1, and see whether the
classifier holds steady at higher rates.
